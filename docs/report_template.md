...
[Three sentences: a Russian APT-style scenario targeting crypto-mining and lateral movement in GCP; what was demonstrated in this lab (escape, control-plane stress, Trivy→BigQuery, vulnerable GitHub Actions WIF); what defensive controls were applied and what risk remains.]
...
- BigQuery dataset `trivy_logs` (location **US**) — tables `raw_compressed_logs` and `clean_vulnerabilities`
- Workload Identity pool `github-pool` with OIDC provider `github-provider` (GitHub Actions)
- Log Router sink `trivy-bq-sink` → BigQuery (after `scripts/log_sink_setup.sh`)
- Service accounts: `trivy-sa` (pod → GCP for BigQuery), `cicd-sa` (GitHub Actions via WIF)

**ASCII diagram (optional):**

```
GitHub PR ──► Actions ──► WIF ──► cicd-sa ──► GCS (tf state) / read-only compute
GKE ──► trivy-operator ──► Cloud Logging ──► Sink ──► BQ raw ──► parse script ──► BQ clean
```
...
4. `google-github-actions/auth` exchanges OIDC for a Google access token for `cicd-sa`.
5. Token can call APIs allowed to that SA (`storage.admin`, `compute.viewer`, etc.).
